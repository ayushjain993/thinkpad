...
import io.uhha.order.vo.ShoppingCartResponse;
...
public class ShoppingCartController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "form", dataType = "array", name = "skuIds", value = "购物车ids"),
    })
    @ApiResponses({
            @ApiResponse(code = 200, message = "成功返回1 失败返回0", response = Integer.class)
    })
    public AjaxResult deleteShoppingCart(@RequestBody Long[] skuIds, HttpServletRequest request) {
        return AjaxResult.success(shoppingCartService.deleteShoppingCart(AppletsLoginUtils.getInstance().getCustomerId(request), skuIds));
    }
