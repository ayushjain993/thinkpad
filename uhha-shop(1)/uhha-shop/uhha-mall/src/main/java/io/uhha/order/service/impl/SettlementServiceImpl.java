...
import io.uhha.marketing.domain.*;
...
import io.uhha.order.service.SettlementService;
import io.uhha.order.service.ShoppingCartServiceApi;
...
public class SettlementServiceImpl implements SettlementService {
    ...
    public OrderSettlement orderSettlement(OrderSettlementRequest orderSettlementRequest) {
        ...
        return OrderSettlement.build(orderSettlementRequest.getIds(),
                orderSettlementRequest.getSkuInfo(),
                customerAddress,
                getStoreSettlements(customerAddress, orderSettlementRequest),
                orderSettlementRequest.getDistributeUserId()).addRedEnvelope(redEnvelopeService.queryCustomerCanUseRedEnvelope(orderSettlementRequest.getCustomerId()));
    }
    ...
    private List<StoreSettlement> getStoreSettlements(UmsMemberAddress customerAddress, OrderSettlementRequest orderSettlementRequest) {
        ...
        // 获得结算信息
        List<StoreSettlement> storeSettlements = convertToStoreSettlements(getShoppingCartResponse(orderSettlementRequest.getIds(), orderSettlementRequest.getSkuInfo(), orderSettlementRequest.getCustomerId(), orderSettlementRequest.getCrowdfundingId(), orderSettlementRequest.getDistributeUserId()));
     ...
     * @return 返回购物车信息
     */
    private List<ShoppingCartResponse> getShoppingCartResponse(Long[] ids, String skuInfo, Long customerId, Long crowdfundingId, Long distributeUseId) {
        ...
        if (!StringUtils.isEmpty(skuInfo)) {
            return shoppingCartServiceApi.queryShoppingCartsBySkuId(skuInfo, customerId, crowdfundingId, distributeUseId);
        }
    ...
    private void setPointInfo(StoreSettlement storeSettlement, Long customerId) {
        ...
        storeSettlement.setPointSetting(pointSettingService.queryPointSetting());
    }
