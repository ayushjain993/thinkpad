...
public class OmsOrderSku extends BaseEntity {
     ...
     * 是否是带货OmsOrderSku
     */
    @Excel(name = "是否第三方带货订单 0 否 1 是默认0")
     ...
     */
    @JsonIgnore
    public boolean hasSetDropshipCommissionRate() {
        return Objects.nonNull(this.dropshipCommissionRate) && this.dropshipCommissionRate.floatValue() != 0;
    }
    ...
    public BigDecimal getDropshipCommission() {
        if (!hasSetDropshipCommissionRate()) {
            return new BigDecimal(0);
        }

        return this.price.multiply(this.dropshipCommissionRate).setScale(2, RoundingMode.HALF_EVEN);
