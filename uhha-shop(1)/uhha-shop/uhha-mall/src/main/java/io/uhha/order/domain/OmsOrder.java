...
import io.uhha.order.vo.SkuResponse;
import io.uhha.order.vo.StoreSettlement;
...
public class OmsOrder extends BaseEntity {
     ...
     * @see io.uhha.common.enums.StatusEnum.YesNoType
     */
    ...
    public boolean isNoNeedPay() {
        return PayTypeEnum.CASH_ON_DELVERY.getCode().equals(this.payType) || checkZeroPrice();
    }
    ...
    public boolean isNotPaid() {
        return OrderStatusEnum.PENDING.getCode().equals(this.status);
    }
    ...
    public void setCreateOrderStatus() {
        // 如果货到付款
        if (PayTypeEnum.CASH_ON_DELVERY.getCode().equals(this.payType)) {
            // 货到付款  则订单状态直接设置成待发货
            this.status = OrderStatusEnum.TO_BE_SHIPPED.getCode();
        } else {
            // 在线支付  则订单状态设置成待付款
            this.status = OrderStatusEnum.PENDING.getCode();
        }
    }
    ...
    @JsonIgnore
    public void setDropshipOrder(StoreSettlement storeSettlement){
        List<SkuResponse> allSkus = storeSettlement.getAllSkus();
        boolean res = allSkus.stream().anyMatch(x-> (BigDecimal.ZERO.compareTo(x.getDropshipCommissionRate())<0));
        this.isDropship = res? "1":"0";
    }
     ...
     * 获得订单的带货佣金
     *
     * @return 返回订单下每个单品的带货佣金
     */
    @JsonIgnore
    public BigDecimal getOrderDropshipCommission() {
        if (CollectionUtils.isEmpty(this.orderSkus) || !orderSkuHasSCommissionRate()) {
            return new BigDecimal(0);
        }
        return this.orderSkus.stream().map(OmsOrderSku::getDropshipCommission).reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    public boolean isPlatform() {
        return CommonConstant.ADMIN_STOREID.equals(this.storeId);
    }
    ...
    public boolean isCommunityBuyOrder() {
        return OrderTypeEnum.COMMUNITY_GROUP_ORDER.getCode().equalsIgnoreCase(this.orderType);
    }
