...
import io.uhha.member.vo.CustomerSearchCondition;
import io.uhha.member.vo.StoreStaff;
...
//        if(customer.isRealNameScreened()){
        customer.setUserBankInfo(userBankInfoService.getUserBankInfoByUid(customer.getId()));
//        }
        return customer.clearSensitiveInfo();
    }
    ...
    public UmsMember queryCustomerWithCustomerLevel(long id) {
        ...
        customer.clearSensitiveInfo().setCustomerLevel(customerLevelService.queryCustomerLevelByMoney(customer.getConsumptionAmount()));
        return customer;
    }
    ...
    public int addCustomer(UmsMember customer) {
        ...
        int insert = customerMapper.insertUmsMember(customer);
        //添加结算账户
        if (insert > 0) {
            SettlementAccount settlementAccount = SettlementAccount.builder()
                    .accountName(customer.getUsername())
                    .accountType(AccountTypeEnum.PERSONAL.name())
                    .associateId(customer.getId())
    ...
    public int updateCustomer(UmsMember customer) {
        ...
        customer.setModifyTime(DateUtils.getNowDate());
        int ret = customerMapper.updateUmsMember(customer);
        if(ret!=1){
            logger.error("updateCustomer fail : db return {} ", ret);
            return ret;
        }

        //更新昵称
        ImUser imUser = imUserService.queryImUserInfo(ImUserTypeEnum.PERSONAL, customer.getId());
        if(imUser!=null) {
            if(StringUtils.isNotEmpty(customer.getNickname())){
                //昵称不同时更新
                if(!customer.getNickname().equalsIgnoreCase(imUser.getNickName())){
                    imUser.setUpdateTime(new Date());
                    imUser.setNickName(customer.getNickname());
                    UpdateWrapper<ImUser> userUpdateWrapper = new UpdateWrapper<>();
                    userUpdateWrapper.eq("id", imUser.getId());
                    imUserService.update(imUser, userUpdateWrapper);
                }
            }else {
                //新昵称为空，更新
                if(StringUtils.isNotEmpty(imUser.getNickName())){
                    imUser.setUpdateTime(new Date());
                    imUser.setNickName(customer.getNickname());
                    UpdateWrapper<ImUser> userUpdateWrapper = new UpdateWrapper<>();
                    userUpdateWrapper.eq("id", imUser.getId());
            ...
            logger.error("imUser not found for umsMember id = {}. invalid data", customer.getId());
        }
        return 1;
    ...
    @Override
    public int updateCustomerFrozenCommission(long customerId, BigDecimal commission){
        logger.debug("updateCustomerFrozenCommission and customerId:{} \r\n commission:{}", customerId, commission);
        Map<String, Object> params = new HashMap<>();
        params.put("customerId", customerId);
        params.put("commissionFrozen", commission);
        return customerMapper.updateCustomerFrozenCommission(params);
    }

    @Override
    public int updateCustomerFrozenCommission2Commission(long customerId, BigDecimal commission){
        logger.debug("updateCustomerFrozenCommission2Commission and customerId:{} \r\n commission:{}", customerId, commission);

        UmsMember member = selectUmsMemberById(customerId);
        if(member==null){
            logger.error("member with customerId:{} not found ", customerId);
            return -1;
        }
        BigDecimal frozenCommission = member.getCommissionFrozen()==null? BigDecimal.ZERO:member.getCommissionFrozen();
        BigDecimal balanceCommission = member.getCommission()==null? BigDecimal.ZERO:member.getCommission();

        //冻结额不够转移
        if(frozenCommission.compareTo(commission)<0){
            logger.error("internal error, not enough balance!");
            return -2;
        }
        ...
        params.put("customerId", customerId);
        params.put("commission", commission);
        return customerMapper.updateCustomerFrozenCommission2Commission(params);
    }
    ...
    private List<UmsMember> clearPassword(List<UmsMember> customers) {
        customers.parallelStream().forEach(customer -> customer.clearSensitiveInfo());
        return customers;
    }
    ...
    public List<AccountUser> queryNotSettlementAccount() {
        return umsMemberMapper.queryNotSettlementAccount();
    }

    @Override
    public UmsMember getUser(Long userId) {
        UmsMember umsMember = umsMemberMapper.selectUmsMemberById(userId);
        return umsMember;
    }

    @Override
    public int changeRealNameStatus(String realnameStatus, Long userId) {
        return umsMemberMapper.changeRealNameStatus(realnameStatus, userId);
    }
}
