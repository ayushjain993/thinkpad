...
import io.uhha.goods.service.SpuServiceApi;
import io.uhha.marketing.domain.Marketing;
import io.uhha.marketing.domain.MarketingItem;
import io.uhha.marketing.service.MarketingQueryService;
import io.uhha.order.domain.OmsShoppingCart;
import io.uhha.order.service.CustomerPanicRecordService;
...
public class ShoppingCartServiceApiImpl implements ShoppingCartServiceApi {
    ...
    @Override
    public List<ShoppingCartResponse> queryShoppingCartsBySkuId(String skuInfo, Long customerId, Long crowdfundingId, Long distributeUseId) {
        logger.debug("queryShoppingCartsBySkuId and skuInfo:{} \r\n customerId:{} \r\n crowdfundingId:{} \r\n distributeUseId:{}", skuInfo, customerId, crowdfundingId, distributeUseId);
        ...
        //设置带货推广用户id
        shoppingCart.setDistributeUserId(distributeUseId);

        if(shoppingCart == null){
            return Collections.emptyList();
        }

        List<ShoppingCartResponse> shoppingCartResponses = getShoppingCartDetails(Arrays.asList(shoppingCart));
        shoppingCartResponses.forEach(x->x.setDistributeUserId(distributeUseId));
