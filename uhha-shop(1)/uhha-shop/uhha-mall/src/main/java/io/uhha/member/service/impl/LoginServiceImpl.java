...
import io.uhha.member.service.LoginService;
...
import io.uhha.member.vo.UnAuthLoginParams;
...
public class LoginServiceImpl implements LoginService {
    ...
    @Override
    public AjaxResult loginWithSms(LoginParams loginParams) {
        ...
        //验证验证码
        if(!validateNotifyHelper.validateSmsCode(customer.getAreacode(),
                customer.getMobile(),
                PlatformEnum.UHHA.getCode(),
                BusinessTypeEnum.SMS_LOGIN.getCode(),
                loginParams.getCode()))
        {
...
//                .claim("releName", customer.getRelename())
        ...
        if (customer != null) {
            ...
            redisService.setCacheObject(sb.toString(), JSON.toJSONString(appletLoginRedisParamResponse),1, TimeUnit.DAYS);
            redisService.setCacheObject(loginParams.getMobile(), sb.toString());
...
//                .claim("releName", customer.getRelename())
    ...
    public int storeAppLogin(String username, String password, Consumer<UmsMember> consumer) {
        ...
        if (Objects.isNull(storeInfo) || StoreStatusEnum.CLOSED.getCode().equalsIgnoreCase(storeInfo.getStatus())
                || !StoreStatusEnum.FILLING.getCode().equalsIgnoreCase(storeInfo.getType())
                || !StoreStatusEnum.APPROVED.getCode().equalsIgnoreCase(storeInfo.getStatus())) {
