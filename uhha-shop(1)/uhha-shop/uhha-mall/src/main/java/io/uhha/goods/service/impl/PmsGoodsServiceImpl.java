...
import io.uhha.goods.mapper.PmsGoodsMapper;
...
public class PmsGoodsServiceImpl implements IPmsGoodsService {
    ...
    @Autowired
    private ITStoreCustomizeBrandService customizeBrandService;
    ...
    public int updatePmsGoods(PmsGoods spu) {
        ...
        //平台商品直接通过
        if (CommonConstant.ADMIN_STOREID.equals(spu.getStoreId())){
            spu.setStatus(AuditStatusEnum.PASS.getCode());
            // 设置商品为审核状态
            spu.setAuditStatus(false);
            // 设置商品下面单品的审核状态和商品一致
            spu.setSkuStatus();
        }else{
            //审核开关打开情况下，则需要判断是否修改了商品如果商品修改了 则商品下的所有单品都需要审核
            if(baseInfoSetService.isSkuNeedAudit()) {
        ...
        //纠正sku名字和同步spu与sku之间的状态
        List<PmsSku> pmsSkuTempList = spu.getSkus().stream().peek(sku -> sku.fixSkuNameForSave(spu.getName())).collect(Collectors.toList());
        List<PmsSku> pmsSkuList = pmsSkuTempList.stream().peek(sku->sku.setStatus(spu.getStatus())).collect(Collectors.toList());
        // 更新单品
        skuService.updateSkus(pmsSkuList, spu.getId(), spu.getStoreId(), sputemp.getCommissionRate());
    ...
    public PageHelper<PmsSku> querySkus(PageHelper<PmsSku> pageHelper, long storeId, String name, String skuNo, PmsSkuItem... skuItemse) {
        ...
        logger.debug("querySkus and pageHelper:{} \r\n  storeId:{} \r\n name: {} \r\n skuNo: {} \r\n skuItemse :{}", pageHelper, storeId, name, skuNo, skuItemse);
    ...
    @Transactional
    @Override
    public int updateSpuDropshipStatus(List<Long> spuIds, String isDropship, long storeId, Consumer<Long> consumer) {
        logger.debug("updateSpuDropshipStatus and spuIds:{} \r\n isDropship:{} \r\n storeId:{}", spuIds, isDropship, storeId);

        if (CollectionUtils.isEmpty(spuIds)) {
            logger.error("updateSpuDropshipStatus fail due to spuIds is empty....");
            return 0;
        }

        // 修改商品的是否带货状态
        updateSpuDropshipStatus(spuIds, isDropship, storeId);

        // 修改单品的是否带货状态
        skuService.updateDropshipStatus(spuIds, isDropship, storeId);
     ...
     * @param storeId 店铺id
     */
    private void updateSpuDropshipStatus(List<Long> spuIds, String status, long storeId) {
        logger.debug("updateSpuDropshipStatus and spuIds:{} \r\n status:{} \r\n storeId:{}", spuIds, status, storeId);
        Map<String, Object> params = new HashMap<>();
        params.put("spuIds", spuIds);
        params.put("status", status);
        params.put("storeId", storeId);
        spuMapper.updateDropshipStatus(params);
    }
    ...
    @Override
    public int updateCommission(long id, BigDecimal commissionRate, BigDecimal sCommissionRate, BigDecimal dropshipCommissionRate,  long storeId) {
        logger.debug("updateCommission and id :{} \r\n commission :{} \r\n sCommissionRate :{} \r\n dropshipCommissionRate:{}\r\nstoreId:{}", id, commissionRate, sCommissionRate, dropshipCommissionRate, storeId);
        Map<String, Object> params = new HashMap<>();
        params.put("id", id);
        params.put("commissionRate", commissionRate);
        params.put("sCommissionRate", sCommissionRate);
        //设置了带货佣金费率则为带货
        if(!BigDecimal.ZERO.equals(dropshipCommissionRate)){
            params.put("dropshipCommissionRate", dropshipCommissionRate);
            params.put("isDropship", "1");
        }else{
            params.put("dropshipCommissionRate", BigDecimal.ZERO);
            params.put("isDropship", "0");
        }
        params.put("storeId", storeId);

        //给单品设置佣金比例
        if (skuService.updateCommission(id, commissionRate, sCommissionRate, dropshipCommissionRate, storeId) == 0) {
    ...
    @Transactional
    @Override
    public int updateCommissions(List<Long> ids, BigDecimal commissionRate, BigDecimal sCommissionRate, BigDecimal dropshipCommissionRate,long storeId) {
        logger.debug("updateCommissions and ids :{} \r\n commissionRate :{} \r\n sCommissionRate :{} \r\n dropshipCommissionRate:{} \r\n storeId", ids, commissionRate, sCommissionRate, dropshipCommissionRate, storeId);

        if (CollectionUtils.isEmpty(ids)) {
            logger.error("updateCommissions fail due to ids is empty");
            return 0;
        }
        ids.stream().forEach(id -> updateCommission(id, commissionRate, sCommissionRate, dropshipCommissionRate, storeId));
        return 1;
    }
     ...
     */
    @Override
    public List<PmsGoods> queryAllStoreAuditdSpusByStatus(String status) {
        logger.debug("queryAllStoreToBeAuditdSpus...");
        PmsGoods query = new PmsGoods();
        if(!AuditStatusEnum.UNDER_AUDIT.getCode().equalsIgnoreCase(status)
                && !AuditStatusEnum.PASS.getCode().equalsIgnoreCase(status)
                &&!AuditStatusEnum.REJECTED.getCode().equalsIgnoreCase(status)){
            logger.warn("invalid status :{}", status);
            return null;
        }
        query.setStatus(status);
        return querySpus(query);
    }
    ...
    private void setBrandAndCategory(PmsGoods spu) {
        if(spu.getCustomizeBrand()==1){
            TStoreCustomizeBrand customizeBrand = customizeBrandService.selectTStoreCustomizeBrandById(spu.getBrandId());
            PmsBrand brand = new PmsBrand();
            BeanUtils.copyProperties(customizeBrand, brand);
            spu.setBrand(brand);
        }else{
