...
import io.uhha.common.enums.CancelReasonEnum;
...
import io.uhha.marketing.domain.RedEnvelopeCode;
import io.uhha.marketing.domain.TrySkuApply;
import io.uhha.marketing.service.*;
...
import io.uhha.util.ThreadTask;
...
public class OrderServiceApiImpl implements OrderServiceApi {
    ...
    public SubmitOrderResponse submitOrder(SubmitOrderParams submitOrderParams) {
        ...
        // 获得订单的每个店铺的结算信息
        OrderSettlement orderSettlement = settlementService.orderSettlement(
                OrderSettlementRequest.build(submitOrderParams.getIds(),
                        submitOrderParams.getSkuInfo(),
                        submitOrderParams.getIsGroup(),
                        submitOrderParams.getGroupId(),
                        submitOrderParams.getAddressId(),
                        submitOrderParams.getCrowdfundingId(),
                        submitOrderParams.getCrowdfundingType(),
                        submitOrderParams.getCrowdfundingMoney(),
                        submitOrderParams.getDistributeUserId()).addCustomerId(submitOrderParams.getCustomerId()));
        ...
        // 返回订单信息
        SubmitOrderResponse submitOrderResponse = getSubmitOrderResponse(orders);

        //发送通知
        mallNotifyHelper.orderSubmitted(submitOrderParams, submitOrderResponse);
    ...
    public int cancleOrder(CancelOrderParams cancelOrderParams) {
        ...
        //发送通知
        mallNotifyHelper.cancelOrder(cancelOrderParams);

        return 1;
    }

    @Override
    public int deleteOrder(DeleteOrderParams deleteOrderParams) {
        logger.debug("deleteOrder and  deleteOrderParams {}", deleteOrderParams);

        if (Objects.isNull(deleteOrderParams)) {
            logger.error("deleteOrder fail due to deleteOrderParams is null...");
            return 0;
        }

        // 查询订单信息
        OmsOrder order = this.queryOrderDetailById(deleteOrderParams.getOrderId(), deleteOrderParams.getCustomerId(), deleteOrderParams.getStoreId(), OrderItem.SKUS);

        // 如果订单来自前端  并且订单的会员id和用户的会员id不一致 则直接返回
        if (deleteOrderParams.isFromCustomer() && deleteOrderParams.getCustomerId() != order.getCustomerId()) {
            logger.error("deleteOrder fail due to order customerId is not match...");
            return -2;
        }

        // 取消订单失败
        if (orderService.deleteOrder(deleteOrderParams.getOrderId(), deleteOrderParams.getStoreId()) == 0) {
            logger.error("deleteOrder fail....");
            return 0;
        }
    ...
    public int cancelDepositPreSaleOrder(CancelOrderParams cancelOrderParams) {
        ...
        //发送通知
        mallNotifyHelper.cancelDepositPreSaleOrder(cancelOrderParams);
    ...
    public int confirmOrderPayed(ConfirmOrderParams confirmOrderParams) {
        ...
        //发送通知
        mallNotifyHelper.confirmOrderPayed(confirmOrderParams);
    ...
    private Consumer<OmsOrder> getCrowdfundingConsumer() {
        return order -> {
            ...
            if (order.isNoReturnOrder()) {
                billingRecordService.insertOmsBillingRecords(OmsBillingRecords.buildForConfirmOrder(setOrderStoreName(order)));
            }
        };
    }
    ...
    public int confirmReceipt(long orderId, long customerId) {
        ...
        if (order.hasRecommendedCommission() && order.isNeedCommission()) {
            ...
            //更新佣金,暂存如冻结余额，到时自动转为可用余额
            customerService.updateCustomerFrozenCommission(order.getRecommended(), order.getOrderCommission());
            ...
            if (order.hasSRecommondedCommission()) {
                ...
                // 更新二级会员佣金
                //更新佣金,暂存如冻结余额，到时自动转为可用余额
                customerService.updateCustomerFrozenCommission(order.getSRecommended(), order.getOrderSCommission());
        ...
        //如果是带货订单
        if (StringUtils.equals("1", order.getIsDropship())) {
            Long distributeUserId = order.getDistributeUserId();
            PlatformFeeSet platformFeeSet = lsPlatformFeeSettingService.queryPlatformFeeSet();
            //判断平台费用设置是否正确
            if(!platformFeeSet.isValidDropshipSetting()|| !platformFeeSet.isValidNFTDropshipSetting()){
                logger.error("invalid platformfee setting: dropship setting is incorrect, make sure platform part and merchant part equals 100! \r\n PlatformFeeSet={}", platformFeeSet);
                return 0;
            }
            //店铺带货
            if(distributeUserId == null){
                //非自营则添加带货提成
                if(order.getStoreId()!= CommonConstant.ADMIN_STOREID){
                    BigDecimal dropshipCommission = order.getOrderDropshipCommission();
                    BigDecimal defaultDropshipMerchantPart = Optional.ofNullable(platformFeeSet).map(PlatformFeeSet::getDefaultDropshipMerchantPart).orElse(BigDecimal.ZERO);
                    BigDecimal defaultDropshipPlatPart = Optional.ofNullable(platformFeeSet).map(PlatformFeeSet::getDefaultDropshipPlatformPart).orElse(BigDecimal.ZERO);
                    BigDecimal merchantPart = MathUtils.mul(dropshipCommission, defaultDropshipMerchantPart);
                    BigDecimal platPart = MathUtils.mul(dropshipCommission, defaultDropshipPlatPart);
                    // 新增带货佣金订单账单记录（用户部分）
                    billingRecordService.insertOmsBillingRecords(OmsBillingRecords.buildForDropshipOrderUser(setOrderStoreName(order),merchantPart));
                    //新增带货佣金订单账单记录（平台部分）
                    billingRecordService.insertOmsBillingRecords(OmsBillingRecords.buildForDropshipOrderPlatform(setOrderStoreName(order),platPart));
                }
            }
            //用户带货
            else{
                ...
                BigDecimal defaultDropshipMerchantPart = Optional.ofNullable(platformFeeSet).map(PlatformFeeSet::getDefaultDropshipMerchantPart).orElse(BigDecimal.ZERO);
                BigDecimal defaultDropshipPlatPart = Optional.ofNullable(platformFeeSet).map(PlatformFeeSet::getDefaultDropshipPlatformPart).orElse(BigDecimal.ZERO);
                BigDecimal userPart = MathUtils.mul(dropshipCommission, defaultDropshipMerchantPart);
                BigDecimal platPart = MathUtils.mul(dropshipCommission, defaultDropshipPlatPart);
                //添加带货佣金记录CommissionRecord
                commissionRecordService.insertOmsCommissionRecords(OmsCommissionRecords.buildForAdd(distributeUserId, userPart, "带货佣金单号:"+order.getOrderCode(), ""));
                //更新佣金,暂存如冻结余额，到时自动转为可用余额
                customerService.updateCustomerFrozenCommission(distributeUserId, userPart);
                // 新增带货佣金订单账单记录（用户部分）
                billingRecordService.insertOmsBillingRecords(OmsBillingRecords.buildForDropshipOrderUser(setOrderStoreName(order), userPart));
                // 新增带货佣金订单账单记录（平台部分）
                billingRecordService.insertOmsBillingRecords(OmsBillingRecords.buildForDropshipOrderPlatform(setOrderStoreName(order),platPart));
            }
        }
        ...
        //发送通知
        mallNotifyHelper.confirmReceipt(orderId, customerId);
    ...
    @Override
    public int deliverOrder(long orderId, long storeId, String waybillCode, String operationName, long companyId) {
        logger.debug("deliverOrder and orderId:{} \r\n storeId:{} \r\n waybillCode:{} \r\n operationName:{} \r\n companyId:{}", orderId, storeId, waybillCode, operationName, companyId);
        ...
        // 判断是否可以发货
        int result = isCanDeliverOrder(orderId);
        ...
        // 发货失败  (这边失败主要是订单状态不对)
        if (orderService.deliverOrder(orderId, storeId, waybillCode, logisticsCompany.getName(), logisticsCompany.getCode()) == 0) {
            logger.error("deliverOrder fail....");
            return 0;
        }

        orderOperatonLogService.insertOmsOrderOperationLog(OmsOrderOperationLog.buildForDeliverOrder(orderId, operationName));

        //发送通知
        mallNotifyHelper.deliverOrder(orderId, waybillCode, logisticsCompany.getName());
    ...
    public int writeOffVirtualOrder(long id, long storeId, String writeOffCode, String operationName) {
        logger.debug("writeOffVirtualOrder and id :{} \r\n storeId:{} \r\n writeOffCode:{} \r\n operationName:{}", id, storeId, writeOffCode, operationName);
    ...
    public int autoConfirmReceipt() {
        ...
        List<OmsOrder> orderList = orderService.queryOrdersForConfirmReceipt(orderSetting.getAutoConfirm());
        if (!CollectionUtils.isEmpty(orderList)) {
            //批量确认收货
    ...
    public int autoCancelOrder() {
        ...
        if (!CollectionUtils.isEmpty(orderList)) {
            //批量取消订单
            orderList.forEach(order -> cancleOrder(CancelOrderParams.buildAutoCancelSource(order.getId(), order.getStoreId(), CancelReasonEnum.SYSTEM_CANCEL.getCode())));
        }
        return 1;
    }
    ...
    private void cancelGroupOrder(GroupOrder groupOrder) {
        ApplyBackOrderParams applyBackOrderParams = ApplyBackOrderParams.builder()
                .customerId(groupOrder.getCustomerId())
                .orderId(groupOrder.getId())
                .description("系统自动申请退款")
                .reason("9")
                .build();

        if (groupOrder.isInGroup()) {
            //申请退款
            backOrderServiceApi.applyRefundOrder(applyBackOrderParams);
        } else {
            //取消订单
            orderService.cancelOrder(groupOrder.getId(), groupOrder.getStoreId(), "0");
        }
    ...
    private OmsOrder getOrderDetail(Long masterOrderId, StoreSettlement storeSettlement, SubmitOrderParams submitOrderParams, UmsMemberAddress customerAddress) {
        ...
        //设置用户带货信息
        order.setDistributeUserId(storeSettlement.getDistributeUserId());

        //设置是否第三方带货订单
        order.setDropshipOrder(storeSettlement);
    ...
    private OmsOrderSku getOrderSku(SkuResponse skuResponse, SkuPriceDetail skuPriceDetail, long customerId) {
        ...
        orderSku.setIsDropship(skuResponse.getIsDropship());
        orderSku.setDropshipCommissionRate(skuResponse.getDropshipCommissionRate());
        // 查询单品信息
        ...
        //填充带货用户的id
        orderSku.setDistributeUserId(skuResponse.getDistributeUserId());
    ...
    private OmsOrderAttr getOrderAttr(SubmitOrderParams submitOrderParams, UmsMemberAddress customerAddress, StoreSettlement storeSettlement) {
        ...
        if (Objects.nonNull(customerAddress)) {
            ...
            orderAttr.setReceiptMobile(customerAddress.getMobile());
            orderAttr.setReceiptPhone(customerAddress.getMobile());
    ...
    private BigDecimal getPointPrice(int points) {
        return pointSettingService.queryPointSetting().getPointPirce(points);
    }
    ...
    private int calcCustomerUsePoints(Long customerId, BigDecimal orderPrice) {
        ...
        return pointSettingService.queryPointSetting().getMaxPoint(orderPrice, customerAllPoint);
    }
