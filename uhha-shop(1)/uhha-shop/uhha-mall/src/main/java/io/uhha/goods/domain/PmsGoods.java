...
public class PmsGoods extends BaseEntity {
    ...
    /**
     * 品牌id，如果是自定义品牌则使用自定义品牌，否则使用公共品牌
     */
    @Excel(name = "品牌id")
     ...
     */
    @Excel(name = "审核状态  状态  0 申请中  1通过 2 拒绝")
    private String status;

    /**
     * 商品上架状态 0 下架  1上架 2违规下架 默认0
     * @see StatusEnum.ShelfType
     */
     ...
     * 设置单品是否带货
     */
    public void setSkuIsDropship() {
        skus.forEach(sku -> sku.setIsDropship(this.getIsDropship()));
    }
    ...
    public void setAuditStatus(boolean isNeedAudit) {
        ...
        if (CommonConstant.ADMIN_STOREID.equals(this.storeId) || !isNeedAudit) {
            this.status = AuditStatusEnum.PASS.getCode();
        } else {
            // 店铺的需要审核
            this.status = AuditStatusEnum.UNDER_AUDIT.getCode();
        }
    }
