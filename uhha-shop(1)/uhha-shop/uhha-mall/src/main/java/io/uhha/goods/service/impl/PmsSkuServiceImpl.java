...
import io.uhha.goods.mapper.PmsSkuMapper;
...
public class PmsSkuServiceImpl implements IPmsSkuService {
    ...
    @Override
    public int updateDropshipStatus(List<Long> spuIds, String isDropship, long storeId) {
        logger.debug("updateDropshipStatus and spuIds:{} \r\n status:{} \r\n storeId:{}", spuIds, isDropship, storeId);

        if (CollectionUtils.isEmpty(spuIds)) {
            logger.error("updateDropshipStatus fail due to spuIds is null...");
            return 0;
        }

        Map<String, Object> params = new HashMap<>();
        params.put("spuIds", spuIds);
        params.put("isDropship", isDropship);
        params.put("storeId", storeId);
        params.put("upTime", LocalDateTime.now());
        return skuMapper.updateDropshipStatus(params);
    }
    ...
    @Override
    public int updateCommission(long spuId, BigDecimal commissionRate, BigDecimal sCommissionRate, BigDecimal dropshipCommissionRate,  long storeId) {
        logger.debug("updateCommission and spuId :{} \r\n commissionRate :{} \r\n sCommissionRate :{} \r\n dropshipCommissionRate:{} \r\n storeId: {}", spuId, commissionRate, sCommissionRate,dropshipCommissionRate, storeId);
        Map<String, Object> params = new HashMap<>();
        params.put("spuId", spuId);
