...
public class SendUtils {
    ...
    private static boolean send253NormalSms(LsSmsSetting smsSetting, ValidateSmsDO vs) {
        try {
            ...
            StringBuffer sb = new StringBuffer(smsSetting.getUrl() + "?");
            // APIKEY
            sb.append("account=" + smsSetting.getAppKey());
            // 用户名
            sb.append("&pswd=" + smsSetting.getAppSecret());
            // 向StringBuffer追加手机号码
     ...
     * @throws Exception 执行异常
     */
    private boolean send253InternationalSms(LsSmsSetting va, ValidateSmsDO vs) {
        try {
            // 创建StringBuffer对象用来操作字符串
            StringBuffer sb = new StringBuffer(va.getUrl() + "?");
            // APIKEY
            sb.append("un=" + va.getAppKey());
            // 用户名
            sb.append("&pw=" + va.getAppSecret());
            // 向StringBuffer追加手机号码
     ...
     * @throws Exception 执行异常
     */
    private boolean sendAliEmail(LsEmailSetting va, ValidateEmailDO ve) {
        boolean flag = true;
