...
import io.uhha.goods.mapper.PmsCategoryMapper;
...
public class PmsCategoryServiceImpl implements IPmsCategoryService {
    ...
    public List<PmsCategory> selectPmsCategoryList(PmsCategory pmsCategory) {
        return categoryMapper.selectPmsCategoryList(pmsCategory);
    }
    /**
     * 根据父级商品分类id查询商品分类
     *
    ...
    public List<PmsCategory> querySpuCategoryByParentId(long parentId, long storeId) {
        ...
        return categoryMapper.querySpuCategoryByParentId(params);
    }
    /**
     * 新增商品分类
     *
    ...
    public int insertPmsCategory(PmsCategory pmsCategory) {
        //删除缓存
        removeL1L2CategoriesCache();
        pmsCategory.setCreateTime(DateUtils.getNowDate());
        return categoryMapper.insertPmsCategory(pmsCategory);
    }
    ...
    public int updatePmsCategory(PmsCategory pmsCategory) {
        //删除缓存
        removeL1L2CategoriesCache();
        return categoryMapper.updatePmsCategory(pmsCategory);
    }
    ...
    public int deletePmsCategoryByIds(Long[] ids) {
        //删除缓存
        removeL1L2CategoriesCache();
        return categoryMapper.deletePmsCategoryByIds(ids);
    }
    ...
    public int deletePmsCategoryById(Long id) {
        //删除缓存
        removeL1L2CategoriesCache();
        return categoryMapper.deletePmsCategoryById(id);
    }
    ...
    public List<PmsCategory> queryAllFirstAndSecondCategory() {
        List<PmsCategory> pmsCategoryList = redisMallHelper.getAllFirstAndSecondCategory();
        if(CollectionUtils.isEmpty(pmsCategoryList)){
            List<PmsCategory> allList = categoryMapper.queryAllFirstAndSecondCategory();
            pmsCategoryList = allList.stream().filter(PmsCategory::isFirstCategory).map(category -> category.setChildCateGory(allList.stream().filter(childCategory -> category.getId().equals(childCategory.getParentId())).collect(Collectors.toList()))
            ).collect(Collectors.toList());
            redisMallHelper.setAllFirstAndSecondCategory(pmsCategoryList);
        }
     ...
     * 删除L1和L2的商品类别缓存
     */
    private void removeL1L2CategoriesCache(){
        redisMallHelper.remove(RedisConstant.ALL_1_2_CATEGORY);
    }
