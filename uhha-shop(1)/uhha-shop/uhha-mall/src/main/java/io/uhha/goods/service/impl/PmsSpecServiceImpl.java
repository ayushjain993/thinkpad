...
import io.uhha.goods.mapper.PmsSpecMapper;
...
public class PmsSpecServiceImpl implements IPmsSpecService {
    ...
    @Override
    public PmsSpec querySpecByName(String name){
        logger.debug("querySpecByName by name :{}", name);

        PmsSpec spec = specMapper.querySpecsByName(name);
     ...
     */
    @Override
    public List<PmsSpec> querySpecByNumIid(String numIid){
        logger.debug("querySpecByName by numIid :{}", numIid);

        List<PmsSpec> specs = specMapper.querySpecByNumIid(numIid);
