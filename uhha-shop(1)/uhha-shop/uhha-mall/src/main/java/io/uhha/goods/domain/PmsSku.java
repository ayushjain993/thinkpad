...
public class PmsSku extends BaseEntity {
    ...
    /**
     * 审核状态  AuditType
     * @see StatusEnum.AuditType
     */
    @Excel(name = "审核状态  0 审核中 1 审核未通过 2 审核通过  ")
     ...
     */
    @Excel(name = "带货佣金比例")
    private BigDecimal dropshipCommissionRate;
    ...
    /**
     * 是否支持第三方带货 0 否 1 是默认0
     */
    @Excel(name = "是否支持第三方带货 0 否 1 是默认0")
    ...
    public boolean validateStatus() {
        return (StatusEnum.ShelfType.AVAILABLE.code()+"").equalsIgnoreCase(this.shelvesStatus);
    }
    ...
    public boolean inAuditedStatus() {
        return (StatusEnum.AuditType.INIT.code()+"").equalsIgnoreCase(status);
    }
    ...
    public boolean validate(boolean isCrowdfunding) {
        // 如果是众筹 则不要校验上下架状态
        if (isCrowdfunding) {
            return (StatusEnum.AuditType.INIT.code()+"").equalsIgnoreCase(this.status);
        }
        return validateStatus() && (StatusEnum.AuditType.SUCCESS.code()+"").equalsIgnoreCase(this.status);
    }
    ...
    public void fixSkuNameForSave(String spuName) {
        if (name.contains("#")) {
            ...
            if (name.equals(spuName)) {
                //!!! 此处代码奇怪，解释下：因为sku名称设计上加括号在首次添加，为避免修改重复加括号，这里去掉remark的括号保证和首次一致
    ...
    public BigDecimal getDropshipCommissionRate() {
        return dropshipCommissionRate;
    }

    public void setDropshipCommissionRate(BigDecimal dropshipCommissionRate) {
        this.dropshipCommissionRate = dropshipCommissionRate;
    }
    ...
    public String toString() {
        return new ToStringBuilder(this, ToStringStyle.MULTI_LINE_STYLE)
                ...
                .append("isDropship", getIsDropship())
                .append("dropshipCommissionRate", getDropshipCommissionRate())
