...
import io.uhha.member.mapper.UmsPreDepositRecordMapper;
...
import io.uhha.member.vo.QueryCriteria;
...
import static io.uhha.coin.common.Enum.validate.BusinessTypeEnum.SMS_MODIFY_TRADE_PASSWORD;
...
public class UmsPreDepositRecordServiceImpl implements IUmsPreDepositRecordService {
    ...
    @Override
    public BigDecimal queryCutomerAllFrozen(long id) {

        UmsPreDepositRecord predepositRecord = predepositRecordMapper.queryLastRecordByCustomerId(id);

        logger.debug("queryCutomerAllFrozen and id :{} \r\n and result:{}", id, predepositRecord);
        ...
        // 返回用户的预存款总金额
        return predepositRecord.getFrozen();
    }
    ...
    public int addPredepositRecord(UmsPreDepositRecord predepositRecord) {
        ...
        if (predepositRecord.isIncome()) {
            ...
            if(TransTypeEnum.WITHDRAW_FAIL.getCode().equalsIgnoreCase(predepositRecord.getTransType())){
                // 恢复用户冻结金额
                BigDecimal currentFrozenMoney = this.queryCutomerAllFrozen(predepositRecord.getCustomerId());
                predepositRecord.setFrozen(currentFrozenMoney.subtract(predepositRecord.getMoney()));
            }
        }
        ...
        if (predepositRecord.isExpenditure()) {
            ...
            //如果是提现则冻结
            if(TransTypeEnum.WITHDRAW.getCode().equalsIgnoreCase(predepositRecord.getTransType())){
                // 用户冻结金额
                BigDecimal currentFrozenMoney = this.queryCutomerAllFrozen(predepositRecord.getCustomerId());
                predepositRecord.setFrozen(currentFrozenMoney.add(predepositRecord.getMoney()));
            }
        }

        //如果是提现结束
        if(TransTypeEnum.WITHDRAW_FINISHED.getCode().equalsIgnoreCase(predepositRecord.getTransType())){
            // 冻结金额抹平
            BigDecimal currentFrozenMoney = this.queryCutomerAllFrozen(predepositRecord.getCustomerId());
            predepositRecord.setFrozen(currentFrozenMoney.subtract(predepositRecord.getMoney()));
        }
    ...
    public int updatePayPassword(UpdatePayPwdBean updatePayPwdBean) {
        ...
        if (CommonConstant.FROM_PC.equals(updatePayPwdBean.getRequestFrom())) {
            ...
            // 校验用户输入的验证码是否正确
            if (!validateNotifyHelper.validateSmsCode(customer.getAreacode(), customer.getMobile(), PlatformEnum.UHHA.getCode(), SMS_MODIFY_TRADE_PASSWORD.getCode(), updatePayPwdBean.getCode())) {
    ...
    public UmsPreDepositRecord selectUmsPreDepositRecordById(Long id) {
        return predepositRecordMapper.selectUmsPreDepositRecordById(id);
    }
    ...
    public List<UmsPreDepositRecord> selectUmsPreDepositRecordList(UmsPreDepositRecord umsPreDepositRecord) {
        return predepositRecordMapper.selectUmsPreDepositRecordList(umsPreDepositRecord);
    }
    ...
    public int insertUmsPreDepositRecord(UmsPreDepositRecord umsPreDepositRecord) {
        ...
        return predepositRecordMapper.insertUmsPreDepositRecord(umsPreDepositRecord);
    }
    ...
    public int updateUmsPreDepositRecord(UmsPreDepositRecord umsPreDepositRecord) {
        return predepositRecordMapper.updateUmsPreDepositRecord(umsPreDepositRecord);
    }
    ...
    public int deleteUmsPreDepositRecordByIds(Long[] ids) {
        return predepositRecordMapper.deleteUmsPreDepositRecordByIds(ids);
    }
    ...
    public int deleteUmsPreDepositRecordById(Long id) {
        return predepositRecordMapper.deleteUmsPreDepositRecordById(id);
    }
}
