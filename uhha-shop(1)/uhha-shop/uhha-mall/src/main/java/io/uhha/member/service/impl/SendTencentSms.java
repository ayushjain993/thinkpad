    ...
    public static int sendMessage(LsSmsSetting smsSet, String[] params, String phones) {
        ...
        try {
            ...
            if (phoneNumbers.length == 1) {
                ...
                SmsSingleSender ssender = new SmsSingleSender(Integer.parseInt(smsSet.getUrl()), smsSet.getAppKey());
                result = ssender.sendWithParam("86", phoneNumbers[0], Integer.parseInt(smsSet.getTemplateId()), params, smsSet.getSign(), "", "");
            } else {
                SmsMultiSender msender = new SmsMultiSender(Integer.parseInt(smsSet.getUrl()), smsSet.getAppKey());
