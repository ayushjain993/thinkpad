...
import io.uhha.common.enums.MemberPriceEnum;
import io.uhha.common.enums.StoreAuditStatusEnum;
...
public class BaseInfoSet implements Serializable {
    ...
    public boolean isMemberPriceOpen() {
        return MemberPriceEnum.ACTIVATED.getCode().equalsIgnoreCase(this.memberPriceStatus);
    }
    ...
    public boolean isSpuNeedAudit() {
        return StoreAuditStatusEnum.NEED_AUDIT.getCode().equalsIgnoreCase(this.storeSpuAudit);
    }
     ...
     * 返回网站地址，末端加 "/"
     */
    public String getSiteBackendDomainWithBias() {
        if (StringUtils.isEmpty(this.h5CallBackDomain)) {
            return null;
        }
        return this.h5CallBackDomain.endsWith("/") ? this.h5CallBackDomain : this.h5CallBackDomain + "/";
    }
