...
import io.uhha.marketing.domain.CouponCode;
...
public class UmsMemberController extends BaseController {
    ...
    public AjaxResult getInfo(@PathVariable("id") Long id) {
        return AjaxResult.success(umsMemberService.queryCustomerWithNoPasswordById(id));
    }
     ...
     * @return 成功1 否则失败  -1
     */
    @GetMapping("/resetGoogleAuth/{customerId}")
    @ApiOperation(value = "管理员重置用户的谷歌认证器", notes = "管理员重置用户的谷歌认证器（需要认证）")
    @PreAuthorize("@ss.hasPermi('member:UmsMember:edit')")
    @ApiResponses({
            @ApiResponse(code = 200, message = "成功1 否则失败  -1 ", response = Integer.class)
    })
    public int resetGoogleAuth(@PathVariable("customerId") Long customerId, HttpServletRequest request) {
        //获取用户信息
        UmsMember umsMember = null;
        try {
            umsMember = umsMemberService.selectUmsMemberById(customerId);
            //验证谷歌
            String ip = Utils.getIpAddr(request);
            umsMember.setGoogleAuthenticator("");
            umsMember.setGoogleUrl("");
            umsMember.setIsGoogleVerification("0");
            ...
            //记录用户事件
            umsMemberChangeHistoryService.logMemberEvent(UmsMemberChangeEventEnum.RESET_BIND_GOOGLE_AUTH, umsMember.getMobile(), AdminLoginUtils.getInstance().getManagerName());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 1;
