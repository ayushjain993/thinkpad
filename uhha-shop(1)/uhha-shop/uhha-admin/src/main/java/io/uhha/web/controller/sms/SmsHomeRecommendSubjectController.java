...
public class SmsHomeRecommendSubjectController extends BaseController {
    ...
    @PostMapping
    public AjaxResult add(@RequestBody List<SmsHomeRecommendSubject> recommendSubjects) {
        int added = 0;
        for (SmsHomeRecommendSubject recommendSubject : recommendSubjects) {
            recommendSubject.setRecommendStatus(StatusEnum.YesNoType.YES.code());
            recommendSubject.setSort(0);
            recommendSubject.setStoreId(CommonConstant.ADMIN_STOREID);
