...
public class PmsGoodsController extends BaseController {
    ...
    public TableDataInfo list(PmsGoods pmsGoods) {
        pmsGoods.setStoreId(AdminLoginUtils.getInstance().getStoreId());
        startPage();
    ...
    public int addSpu(@RequestBody PmsGoods spu) {
        spu.setStoreName(AdminLoginUtils.getInstance().getStoreName());
        return pmsGoodsService.insertPmsGoods(spu.setDefaultValuesForAdd(AdminLoginUtils.getInstance().getManagerName(), AdminLoginUtils.getInstance().getStoreId()));
    }
    ...
    public List<PmsBrand> queryBrands() {
        return brandService.queryAllBrands(AdminLoginUtils.getInstance().getStoreId());
    }
    ...
    public int updateShelvesStatus(Long[] ids, String status) {
        return spuService.updateShelvesStatus(Arrays.asList(ids), status, AdminLoginUtils.getInstance().getStoreId(), null);
    }
     ...
     * @return 成功返回 1 失败返回0
     */
    @DeleteMapping("/spus/{ids}")
    @ApiOperation(value = "批量删除商品信息", notes = "批量删除商品信息（需要认证）")
    @PreAuthorize("@ss.hasPermi('goods:goods:remove')")
    public int deleteSpus(@PathVariable Long[] ids) {
        return spuService.deleteSpus(Arrays.stream(ids).map(id -> PmsGoods.buildForDelete(AdminLoginUtils.getInstance().getManagerName(), id, AdminLoginUtils.getInstance().getStoreId())).collect(Collectors.toList()));
    }
    ...
    public List<OmsLogisticsTemplate> queryAllLogisticsTemplates() {
        return logisticsTemplateService.queryAllTemplates(AdminLoginUtils.getInstance().getStoreId());
    }
     ...
     * 查询供商家带货的商品列表
     * @param pageHelper
     * @param spuSearchCondition
     * @return
     */
    @GetMapping("/spu/dropship")
    @ApiOperation(value = "查询供商家带货的商品列表", notes = "查询供商家带货的商品列表（需要认证）")
    public PageHelper<PmsGoods> queryDropshipSimpleSpus(@ApiIgnore PageHelper<PmsGoods> pageHelper, @ApiIgnore SpuSearchCondition spuSearchCondition) {
